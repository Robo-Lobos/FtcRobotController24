 ...
 */
@Autonomous(name = "InnerRedAuto")

class InnerBlueAuto : LinearOpMode() {
    /**
            ...
            while (opModeIsActive()) {
                ...
                when (elementPosition) {
                    1 -> {
                        ...
                        drive.followTrajectory(Trajectories.rrtraj.innerred1)
                    //CHANGE PER FILE
                        //common traj, then specific traj for color/side // THIS IS STUPID, EASIER TO WRITE 12 PATHS
                    }
                    2 -> {
                        telemetry.addData("logic test", elementPosition)
                        drive.followTrajectory(Trajectories.rrtraj.innerred2)//
                        //separate teleop functions by color and side for easy driver use
                    }
                    3 -> {
                        telemetry.addData("logic test", elementPosition)
                        drive.followTrajectory(Trajectories.rrtraj.innerred3)
                        //drive.turn(Math.toRadians(-90.0))
                        //drive.followTrajectory(Trajectories.rrtraj.innerred3a)//
                    }
                    else -> {
                        //share the CPU.
                        sleep(20)
